...
COMMENT ON COLUMN biosec.scorecard_item.is_killer_related IS $$ Đánh dấu tiêu chí này có liên quan tới killer metric hay không. $$;
...
COMMENT ON COLUMN biosec.killer_metric_event.source_type IS $$ Nguồn phát hiện: assessment, audit, field report... $$;
COMMENT ON COLUMN biosec.killer_metric_event.summary IS $$ Mô tả ngắn nội dung vi phạm hoặc sự kiện. $$;
COMMENT ON COLUMN biosec.killer_metric_event.status IS $$ Trạng thái xử lý sự kiện killer metric. $$;
